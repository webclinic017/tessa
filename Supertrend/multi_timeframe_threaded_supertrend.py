...
candle_writers = {}
tick_writers = {}
tick_frames = [96, 144, 161, 210]

for instrument_token in watchlist:

    ticks_store[instrument_token] = {}
    volume[instrument_token] = {}
    candles[instrument_token] = {}
    candle_writers[instrument_token] = {}
    tick_writers[instrument_token] = csv.writer(
        open(tickertape[instrument_token] + "_ticks.csv", "w"))

    for tick_size in tick_frames:
        ticks_store[instrument_token][tick_size] = []
        volume[instrument_token][tick_size] = 0
        candles[instrument_token][tick_size] = pd.DataFrame(kite.historical_data(
        instrument_token, previous_trading_day + " 15:00:00", previous_trading_day + " 15:21:00", "minute"))
        candle_writers[instrument_token][tick_size] = csv.writer(
        open(tickertape[instrument_token] + '_' + str(tick_size) + ".csv", "w"))
...
def on_candle(instrument_token, ticks, candles, candle_writer, volume):
    ...
    candle_writer.writerow(candle_data)
...
def on_ticks(ws, ticks):
    ...
    for tick in ticks:
        ...
        for tick_size in tick_frames:
            ...
            ticks_store[instrument_token][tick_size].append(ltp)
            volume[instrument_token][tick_size] += last_quantity

            if(len(ticks_store[instrument_token][tick_size]) == tick_size):

                start_new_thread(on_candle, (instrument_token, ticks_store[instrument_token][tick_size], candles[instrument_token][tick_size], candle_writers[instrument_token][tick_size], volume[instrument_token][tick_size]))
