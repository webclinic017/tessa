...
for instrument_token in watchlist:
    ...
    previous_session_ohlc = kite.historical_data(instrument_token, today - timedelta(days=21), today, "day")[-1]

    for ratio in ratios:
        retracement = previous_session_ohlc['high'] - (previous_session_ohlc['high'] - previous_session_ohlc['low']) * ratio
                ...
                if penultimate_candle.close >= retracement_value:
                    if last_candle.high >= penultimate_candle.high:
                        if last_candle.low >= penultimate_candle.low:
                            ...
                            print(f"{ratio} retracement - {tradingsymbol} at {timestamp} ltp: {last_traded_price}")
                            tradebook.write(f"{ratio} retracement - {tradingsymbol} at {timestamp} ltp: {last_traded_price}")
    ...
    if instrument_token in open_trades:
        if last_candle.high <= penultimate_candle.high:
            if last_candle.low <= penultimate_candle.low:
                ...
                timestamp = get_timestamp()
                last_traded_price = get_ltp(instrument_token)
                print(f"Sell signal - {tradingsymbol} at {timestamp} ltp: {last_traded_price}")
                tradebook.write(f"Sell signal - {tradingsymbol} at {timestamp} ltp: {last_traded_price}")
...
def on_ticks(ws, ticks):
    ...
    for tick in ticks:
        ...
        if(len(ticks210[instrument_token]) == 161):
            ...
            start_new_thread(on_candle, (instrument_token, ticks210[instrument_token], candles[instrument_token], volume))
